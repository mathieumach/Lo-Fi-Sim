...
class interactivePlot(object):   
    def __init__(self,fig, ax, X, TR, TE, T1, T2, M0, flipMap, alpha_orig, B1, noise, data_mat, plotAxis = 2, axisLabels = None, fov = None, cmap = 'gray'):
...
#        set various event handlers
        ...
        # Alpha
        self.axSlider_alp = fig.add_axes([0.15, 0.02, 0.6, 0.03])
        self.slider_alp = Slider(ax=self.axSlider_alp, label='Alpha', valmin=0, valmax=180, valinit=self.alpha_orig) 
        ...
        def update(val):
            ...
            # Alpha
            if self.slider_alp.val == 0:
                alp = self.alpha_orig
            else:
                alp = self.slider_alp.val
                
            anglemap = flipMap/alp  
            
            if te < tr:
                self.X = np.multiply(Gradient_seq(tr, te, self.T1, self.T2, self.M0, anglemap),self.B1) 
        ...
        self.slider_alp.on_changed(update)
...
#        print('Button pressed: %s at X: %s, Y: %s'%(event.button, event.xdata, event.ydata))
        if event.button == 3:   #Reset image on right click
            if self.index == 0:
                self.y_ind = self.data_mat[0] - round(event.xdata)
                ...
                if event.xdata != None:
                    if event.ydata != None:
                        ...
                        if self.index == 0:  
                            ...
                            self.y_ind = self.data_mat[0] - int(event.xdata)
                            imageDataY = np.rot90(self.X[:,self.y_ind,:])
                            if self.y_ind >= np.size(self.X,1):
                                self.y_ind = np.size(self.X,1)-1
                            if self.axisLabels != None:
