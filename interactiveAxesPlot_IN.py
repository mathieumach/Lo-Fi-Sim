...
class interactivePlot(object):   
    def __init__(self,fig, ax, X, TR, TE, TI, T1, T2, M0, B1, trajectory, Readout_axis, ETL, noise, data_mat, plotAxis = 2, axisLabels = None, fov = None, cmap = 'gray'):
...
#        set various event handlers
        ...
        #create sliders
        # TR
        self.axSlider_TR = fig.add_axes([0.15, 0.12, 0.6, 0.03])
        self.slider_TR = Slider(ax=self.axSlider_TR, label='TR', valmin=0, valmax=15000, valinit=self.TR*1000)
        # TE
        self.axSlider_TE = fig.add_axes([0.15, 0.08, 0.6, 0.03])
        self.slider_TE = Slider(ax=self.axSlider_TE, label='TE', valmin=0, valmax=1000, valinit=self.TE*1000)
        # TI
        self.axSlider_TI = fig.add_axes([0.15, 0.04, 0.6, 0.03])
        self.slider_TI = Slider(ax=self.axSlider_TI, label='TI', valmin=0, valmax=5000, valinit=self.TI*1000) 
        # ETL
        ...
        def update(val):               
            ...
            if te < tr:
                self.X = np.multiply(IN_seq(tr, ti, self.T1, self.T2, self.M0), self.B1)    
                ...
                if event.xdata != None:
                    if event.ydata != None:
                        ...
                        if self.index == 0:  
                            ...
                            self.z_ind = self.data_mat[2] - int(event.ydata)
                            imageDataZ = np.rot90(self.X[:,:,self.z_ind])
                            if self.z_ind >= np.size(self.X,2):
                                self.z_ind = np.size(self.X,2)-1
                            if self.axisLabels != None:
                            ...
                            self.y_ind = self.data_mat[1] - int(event.ydata)
                            imageDataY = np.rot90(self.X[:,self.y_ind,:])
                            if self.y_ind >= np.size(self.X,1):
                                self.y_ind = np.size(self.X,1)-1
                            if self.axisLabels != None:
