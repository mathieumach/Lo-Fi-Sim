...
class interactivePlot(object):   
    def __init__(self,fig, ax, X, TR, TE, T1, T2, ETL, trajectory, readout, M0, B1, noise, data_mat, plotAxis = 2, axisLabels = None, fov = None, cmap = 'gray'):
...
#        set various event handlers
        ...
        #create sliders
        if self.traj == 'Out-in' or self.traj == 'Linear':      
            # TR
            self.axSlider_TR = fig.add_axes([0.15, 0.1, 0.6, 0.03])
            self.slider_TR = Slider(ax=self.axSlider_TR, label='TR', valmin=0, valmax=5000, valinit=self.TR*1000)
            # TE
            self.axSlider_TE = fig.add_axes([0.15, 0.06, 0.6, 0.03])
            ...
            self.axSlider_ETL = fig.add_axes([0.15, 0.02, 0.6, 0.03])
            self.slider_ETL = Slider(ax=self.axSlider_ETL, label='ETL', valmin=0, valstep = allowed_ETL, valmax = allowed_ETL[-1], valinit=self.ETL)  
            ...
            def update(val):
                ...
                if te < tr:
                    ...
                    if self.traj == 'Out-in':    
                        TEeff = te * etl    
                        self.X = spin_echo_seq(tr, TEeff, self.T1, self.T2, self.M0) # Computing a spin echo
                    elif self.traj == 'In-out':
                        TEeff = te
                        TE_div = np.divide(-TEeff, self.T2, out=np.zeros_like(self.T2), where=self.T2!=0)
                        self.X = np.abs(self.M0 * np.exp(-TE_div))                         # Computing a spin echo with the (1 - exp(-TR/T1)) set to 1
                    elif self.traj == 'Linear':
                        TEeff = 0.5 * te * etl
                        self.X = spin_echo_seq(TR, TEeff, self.T1, self.T2, self.M0) # Computing a spin echo
                    
                    self.X = TSE_seq_R(te, etl, self.T2, self.traj, self.readout, self.X)
                    self.X = np.multiply(self.X, self.B1)
            ...
            def update(val):
                ...
                if self.slider_ETL.val == 0:
                    ...
                    etl = self.slider_ETL.val

                if te < self.TR:
                    self.X = TSE_seq(self.TR, te, etl, self.M0, self.T1, self.T2, self.c, self.traj)
                    self.X = np.multiply(self.X, self.B1)
                    self.X = self.X + self.noise
                    if self.index == 0:
                        self.fig.suptitle("Action will be used on sagittal axis", fontsize=14)
