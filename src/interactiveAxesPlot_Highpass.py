...
class interactivePlot(object):   
    def __init__(self,fig, ax, X, kernel_size, data_mat, plotAxis = 2, axisLabels = None, fov = None, cmap = 'gray'):
...
#        set various event handlers
        ...
        self.X = cv2.Laplacian(self.original, -1, ksize = self.kernel_size)  # The -1 is so that the final image as the same depth as the original one
        ...
        # create slider        
        # kernel size
        allowed_kernel_size = np.arange(1,19,2)
        self.axSlider_kernel_size = fig.add_axes([0.15, 0.02, 0.6, 0.03])
        self.slider_kernel_size = Slider(ax=self.axSlider_kernel_size, label='kernel size', valmin=1, valstep = allowed_kernel_size, valmax = allowed_kernel_size[-1], valinit=self.kernel_size)  
        
        def update(val):
            
            # kernel size
            if self.slider_kernel_size.val == 0:
                ks = self.kernel_size
            else:
                ks = self.slider_kernel_size.val
            
            ks = int(ks)
      
            self.X = cv2.Laplacian(self.original, -1, ksize = ks)
        ...
        self.slider_kernel_size.on_changed(update)
