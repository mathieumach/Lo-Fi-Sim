...
class interactivePlot(object):   
    def __init__(self,fig, ax, X, TR, TE, TI, TI2, T1, T2, M0, B1, trajectory, Readout_axis, ETL, noise, data_mat, plotAxis = 2, axisLabels = None, fov = None, cmap = 'gray'):
...
#        set various event handlers
        ...
        #create sliders
        # TR
        self.axSlider_TR = fig.add_axes([0.15, 0.15, 0.6, 0.03])
        self.slider_TR = Slider(ax=self.axSlider_TR, label='TR', valmin=0, valmax=15000, valinit=self.TR*1000)
        # TE
        self.axSlider_TE = fig.add_axes([0.15, 0.11, 0.6, 0.03])
        self.slider_TE = Slider(ax=self.axSlider_TE, label='TE', valmin=0, valmax=5000, valinit=self.TE*1000)
        # TI
        self.axSlider_TI = fig.add_axes([0.15, 0.07, 0.6, 0.03])
        self.slider_TI = Slider(ax=self.axSlider_TI, label='TI', valmin=0, valmax=10000, valinit=self.TI*1000)
        # TI2
        self.axSlider_TI2 = fig.add_axes([0.15, 0.04, 0.6, 0.03])
        self.slider_TI2 = Slider(ax=self.axSlider_TI2, label='TI2', valmin=0, valmax=10000, valinit=self.TI2*1000)
        # ETL
        allowed_ETL = np.concatenate([np.linspace(0, 100, 101)])
        self.axSlider_ETL = fig.add_axes([0.15, 0.01, 0.6, 0.03])
        self.slider_ETL = Slider(ax=self.axSlider_ETL, label='ETL', valmin=1, valstep = allowed_ETL, valmax=100, valinit=self.ETL) 
        ...
        def update(val):               
            ...
            # TI
            if self.slider_TI.val == 0:
                ti = self.TI
            else:
                ti = np.divide(self.slider_TI.val,1000)
            # TI2
            if self.slider_TI2.val == 0:
                ti2 = self.TI2
            else:
                ti2 = np.divide(self.slider_TI2.val,1000)
            # ETL
            if self.slider_ETL.val == 0:
                etl = self.ETL
            else:
                etl = int(self.slider_ETL.val)
                
            if te < tr:
                self.X = np.multiply(DoubleInversion_seq(TR, te, ti, ti2, self.T1, self.T2, self.M0),self.B1) 
                
                if self.trajectory == 'Out-in':    
                    TEeff = te * etl    
                    TE_div = np.divide(-TEeff, self.T2, out=np.zeros_like(self.T2), where=self.T2!=0)
                    self.X = self.X*np.exp(TE_div)
                elif self.trajectory == 'In-out':
                    TEeff = te
                    TE_div = np.divide(-TEeff, self.T2, out=np.zeros_like(self.T2), where=self.T2!=0)
                    self.X = self.X*np.exp(TE_div)
                ...
                self.X = TSE_seq_R(te, etl, self.T2, self.trajectory, self.readout_axis, self.X) 
        ...
        self.slider_TI.on_changed(update)
        self.slider_TI2.on_changed(update)
        self.slider_ETL.on_changed(update)
