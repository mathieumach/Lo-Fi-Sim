...
class interactivePlot(object):   
    def __init__(self,fig, ax, X, TR, TE, T1, T2, M0, b, D, B1, trajectory, Readout_axis, ETL, noise, data_mat, plotAxis = 2, axisLabels = None, fov = None, cmap = 'gray'):
...
#        set various event handlers
        ...
        #create sliders
        # TR
        self.axSlider_TR = fig.add_axes([0.15, 0.10, 0.6, 0.03])
        self.slider_TR = Slider(ax=self.axSlider_TR, label='TR', valmin=0, valmax=10000, valinit=self.TR*1000)
        # TE
        self.axSlider_TE = fig.add_axes([0.15, 0.07, 0.6, 0.03])
        self.slider_TE = Slider(ax=self.axSlider_TE, label='TE', valmin=0, valmax=1000, valinit=self.TE*1000)
        # b
        self.axSlider_b = fig.add_axes([0.15, 0.04, 0.6, 0.03])
        self.slider_b = Slider(ax=self.axSlider_b, label='b', valmin=0, valmax=1500, valinit=self.b)
        # ETL
        ...
        def update(val):
            ...
            if te < tr:
                self.X = np.multiply(Diffusion_seq(tr, self.T1, self.T2, self.M0, b, self.D),B1)
        ...
        self.slider_b.on_changed(update)
