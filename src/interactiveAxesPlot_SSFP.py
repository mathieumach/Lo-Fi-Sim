...
class interactivePlot(object):   
    def __init__(self,fig, ax, X, TR, T1, T2, M0, alpha, phi, offset, B1, noise, data_mat, plotAxis = 2, axisLabels = None, fov = None, cmap = 'gray'):  
...
#        set various event handlers
        ...
        self.slider_TR = Slider(ax=self.axSlider_TR, label='TR', valmin=0, valmax=100, valinit=self.TR*1000)
        ...
        def update(val):           
            ...
            self.phi = self.offset * tr
                
            #self.X = np.multiply(SSFP_Echo_seq(self.T1, self.T2, self.M0, self.alpha, self.phi),B1)
            self.X = np.multiply(SSFP_new(tr, self.T1, self.T2, self.M0, self.alpha, self.phi),B1)
